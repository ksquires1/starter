...
const employeeContact = data.map(user => [user.website, user.email]);
...
const nineList = dataTwo.filter(user => user.userId === 9);
console.log(nineList);

//then find the number of user 9's incomplete tasks
const incompleteTast = dataTwo.filter(user => user.completed === false);
...
const fiveList = dataThree.filter(user => user.userId === 5);
console.log(fiveList);

//then, create a list of title and body pairs
const fiveTitleBody = dataThree.map(user => [user.title, user.body]);
console.log(fiveTitleBody);
